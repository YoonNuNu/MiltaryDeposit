...
public class Bs extends Calculation {
...
//      우대금리 설정
        double[] treatRates = {0.1};
        double totalTreatment = 0;

        String[] options = {
                "만기해지시 부산은행 주택청약종합저축(청년우대형 포함)보유 고객"
        };
...
//      기준값 벗어날 시 처음으로 반복
        while (true) {
            ...
            if (payment <= 200000 && payment >= 10000) {
                break;
            }
...
//      소수점 없애기
        ...
        System.out.println("고객님의 부산은행 장병내일준비적금 만기 시 세전 금액은 " + intAmount + "입니다.");
        System.exit(0);
    }//calculate
}//class